...
func ParseTime(input string) (Time, error) {
	...
	if len(components) != 3 {
		...
		if hour > 23 || hour < 0 {
			return Time{}, &TimeParseError{"hour out of the range: 0 <= hour <= 23", fmt.Sprintf("%v", hour)}
		}
		if minute > 59 || minute < 0 {
			return Time{}, &TimeParseError{"minute out of range: 0 <= minute <= 59", fmt.Sprintf("%v", minute)}
		}
		if second > 59 || second < 0 {
			return Time{}, &TimeParseError{"minute out of range: 0 <= second <= 59", fmt.Sprintf("%v", second)}
		}
